...
const REMOTE_COLUMNS = {
  products: [
    'id', 'shop_id', 'code', 'name', 'purchase_price', 'sale_price',
    'stock_initial', 'alert_threshold', 'supplier', 'unit',
    'created_at', 'updated_at', 'deleted_at',
  ],
  purchases: [
    'id', 'shop_id', 'date', 'supplier_id', 'supplier',
  ...
  sales: [
    'id', 'shop_id', 'session_id', 'sale_batch_id',
    'date', 'store', 'client_id', 'client_name',
    'payment_status', 'paid_amount', 'remaining_amount',
    'product_id', 'product_code', 'product_name',
    'quantity', 'unit_sale_price', 'total_sale',
    'unit_purchase_cost', 'total_purchase_cost', 'profit',
    'created_at', 'updated_at', 'deleted_at', 'cancelled_at',
  ],
  expenses: [
    'id', 'shop_id', 'date', 'description', 'amount', 'category',
    'created_at', 'updated_at', 'deleted_at',
  ],
  clients: [
    'id', 'shop_id', 'name', 'address', 'phone', 'email',
    'created_at', 'updated_at', 'deleted_at',
  ],
  client_transactions: [
    'id', 'shop_id', 'client_id', 'date', 'label',
  ...
  supplier_transactions: [
    'id', 'shop_id', 'supplier_id', 'date', 'label',
    'quantity', 'unit_amount', 'amount',
    'created_at', 'updated_at', 'deleted_at',
  ],
  invoices: [
    'id', 'shop_id', 'invoice_number', 'type',
    'client_id', 'client_name', 'client_address', 'client_phone',
    'date', 'city', 'total_amount', 'amount_in_words',
    'status', 'created_at', 'updated_at',
  ],
  invoice_items: [
    'id', 'invoice_id', 'designation', 'quantity',
...
async function getRemoteStock(supabase, shopId, salePayload) {
  ...
  const bought = (purchasesRes.data || [])
    ...
    for (const item of queue) {
      try {
        ...
        if (operation === 'delete') {
          ...
          if (table_name === 'sales' && !cleanPayload.cancelled_at && !cleanPayload.deleted_at) {
            const remoteAvailable = await getRemoteStock(supabase, shopId, cleanPayload)
