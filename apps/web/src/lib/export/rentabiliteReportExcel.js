...
function productKey(row) {
  return row.product_id || row.id || row.product_code || row.code || row.product_name || row.name
}
...
export function exportRentabiliteReportExcel({
  ...
  // Produits
  addSheet(wb, 'Produits', [
    ['Code Produit', 'Nom Produit', 'Prix Achat', 'Prix Vente', 'Stock Initial', 'Seuil Alerte', 'Fournisseur'],
    ...products.map(p => [
      p.code || '',
      p.name || '',
      n(p.purchase_price),
  ...
  // Achats
  addSheet(wb, 'Achats', [
    ['Date', 'Fournisseur', 'Code Produit', 'Nom Produit', 'Quantité', 'PA Unitaire', 'Total Achat'],
  ...
  // Ventes
  addSheet(wb, 'Ventes', [
    ['Date', 'Magasin', 'Code Produit', 'Nom Produit', 'Quantité', 'Prix Vente Unitaire', 'Total Vente', 'Coût Unitaire Achat', 'Coût Total Achat', 'Résultat'],
    ...filteredSales.map(s => [
      excelDate(s.date),
      s.store || '',
      s.product_code || '',
  ...
  const stockRows = productStats.map(p => {
    const stockDisponible = p.stockInitial + p.totalAchatsQty - p.totalVentesQty
  ...
  addSheet(wb, 'Stock Intelligent', [
    ['Code Produit', 'Nom Produit', 'Stock Initial', 'Total Achats', 'Total Ventes', 'Stock Disponible', 'Alerte Stock', 'PA Unitaire', 'Stock en valeur'],
    ...stockRows,
  ], {
  ...
  // Analyse Rentabilité
  addSheet(wb, 'Analyse Rentabilité', [
    ['Code Produit', 'Nom Produit', "Chiffre d'Affaires", 'Coût Total Achat', 'Marge Brute', 'Taux de Marge (%)'],
    ...productStats.map(p => [
      p.code,
      p.name,
      p.ca,
  ...
  const totalAchats = filteredPurchases.reduce((a, p) => a + n(p.total_amount), 0)
  ...
  for (let r = 1; r <= 6; r++) {
    const a = `A${r + 1}`
    const b = `B${r + 1}`
    if (dashboard[a]) dashboard[a].s = { font: { bold: true }, border: thinBorder(), fill: { fgColor: { rgb: LIGHT_BLUE } } }
    if (dashboard[b]) dashboard[b].s = { border: thinBorder(), numFmt: '#,##0' }
  }
  ...
  addSheet(wb, 'Créances Clients', [
    ['Date', 'Client', 'Adresse', 'Libellé', 'Quantité', 'PA Unitaire', 'Montant Achat'],
  ...
  addSheet(wb, 'Dettes Fournisseurs', [
    ['Date', 'Fournisseur', 'Adresse', 'Libellé', 'Quantité', 'PA Unitaire', 'Montant Achat'],
  ...
  addSheet(wb, 'Synthese_Mensuelle', [
    ['Mois', 'Total Ventes', 'Coût Total Achat', 'Bénéfice'],
    ...monthlyRows,
  ], {
    widths: [18, 20, 22, 18],
  })
    ...
    for (let r = range.s.r; r <= range.e.r; r++) {
      for (let c = range.s.c; c <= range.e.c; c++) {
        ...
        if (typeof cell.v === 'number' && cell.v < 0) {
          cell.s = {
            ...(cell.s || {}),
            font: { ...(cell.s?.font || {}), color: { rgb: RED }, bold: c >= 4 },
          }
        }
