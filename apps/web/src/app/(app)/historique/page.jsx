...
import {
    PageHeader, SearchBar, EmptyState, StatCard, Badge, selectCls,
} from '@/components/ui'
...
const TYPE_FILTERS = [
    ...
    { value: 'purchase', label: 'Entrées de stock' },
    { value: 'expense', label: 'Charges' },
    { value: 'client', label: 'Mouvements clients' },
    { value: 'supplier', label: 'Mouvements fournisseurs' },
    { value: 'invoice', label: 'Documents' },
]

function matchesTypeFilter(eventType, filterValue) {
    if (filterValue === 'all') return true
    if (filterValue === 'client') return eventType === 'client_payment' || eventType === 'client_credit'
    if (filterValue === 'supplier') return eventType === 'supplier_payment' || eventType === 'supplier_debt'
    if (filterValue === 'sale') return eventType === 'sale' || eventType === 'sale_advance' || eventType === 'sale_cancelled'
...
export default function HistoriquePage() {
    ...
    const events = useMemo(() => {
        ...
        // ── Sales, grouped by session so one "vente" = one event (not one per line) ──
        const saleGroups = {}
        sales.forEach(s => {
        ...
        // ── Client transactions: positive amount = créance, negative = paiement ──
        clientTx.forEach(t => {
        ...
        // ── Supplier transactions: positive amount = dette, negative = paiement ──
    ...
    const filtered = useMemo(() => {
        ...
        return events.filter(ev => {
            const matchesType = matchesTypeFilter(ev.type, typeFilter)
            const matchesSearch = !q ||
                ev.title?.toLowerCase().includes(q) ||
                ev.subtitle?.toLowerCase().includes(q)
            const matchesDate = isDateInFilter(ev.date, dateFilter)
            return matchesType && matchesSearch && matchesDate
        })
    }, [events, search, typeFilter, dateFilter])

    const stats = useMemo(() => {
        const salesCount = filtered.filter(e => e.type === 'sale').length
        const purchaseTotal = filtered.filter(e => e.type === 'purchase').reduce((a, e) => a + e.amount, 0)
        const expenseTotal = filtered.filter(e => e.type === 'expense').reduce((a, e) => a + e.amount, 0)
        return { total: filtered.length, salesCount, purchaseTotal, expenseTotal }
    }, [filtered])
    ...
    return (
        <div className="p-6">
            <PageHeader
                title="Historique"
                subtitle={`${filtered.length} événement${filtered.length !== 1 ? 's' : ''} — toutes opérations confondues`}
            />
            ...
            <div className="card overflow-hidden">
                <div className="flex flex-wrap items-end gap-3 px-5 py-4 border-b border-gray-100">
                    <div className="flex-1 min-w-[220px] max-w-xs">
                        <SearchBar value={search} onChange={setSearch} placeholder="Rechercher dans l'historique…" />
                    </div>

                    <div>
                        <label className="block text-[11px] font-semibold text-gray-400 mb-1">Type</label>
                        ...
                        {filtered.map(ev => {
                            ...
                            return (
                                <div key={ev.id} className="flex items-start gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-none ${meta.color === 'blue' ? 'bg-blue-50 text-blue-600' :
                                            meta.color === 'green' ? 'bg-emerald-50 text-emerald-600' :
