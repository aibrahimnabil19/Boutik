...
import {
  Phone, Mail, FileText, Palette, Upload,
...
const inputClass = `w-full h-11 px-4 bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-400
...
function FileUploadBox({ label, file, onPick }) {
  ...
  return (
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">{label} (optionnel)</label>
      <div
      ...
      <div className="relative w-full max-w-lg">
        ...
        <div className="mb-6">
          <div className="flex justify-between mb-2">
            {STEPS.map((s, i) => (
              <span key={i} className={`text-xs font-medium ${i <= step ? 'text-blue-400' : 'text-slate-600'}`}>{s}</span>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {step === 0 && (
              <div className="space-y-5">
                ...
                {/* Logo upload */}
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-2">Logo (optionnel)</label>
                  <div
                  ...
                  >
                    {logoPreview ? (
                      <Image src={logoPreview} alt="Logo" width={48} height={48} className="w-12 h-12 rounded-lg object-contain bg-white/10" />
                    ) : (
                ...
                <Field label="Nom de la boutique *" error={errors.name?.message}>
                  <input {...register('name', { required: 'Nom requis' })} placeholder="Ex: ELSO Niger" className={inputClass} />
                </Field>
                <Field label="Ville">
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <FileUploadBox label="Photo du cachet / tampon" file={cachetFile}
                  onPick={(f) => handleFileChange(f, setCachetFile, null)} />
                <FileUploadBox label="Photo de la signature" file={signatureFile}
              ...
              <div className="space-y-5">
                ...
                <div className="border-t border-white/10 pt-4">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="color" value={customPrimary || selectedTheme.primary}
                      onChange={(e) => setCustomPrimary(e.target.value)}
                      className="w-10 h-10 rounded-lg cursor-pointer border-0 bg-transparent" />
                    <span className="text-slate-300 text-sm font-mono">{customPrimary || selectedTheme.primary}</span>
                    {customPrimary && (
                      <button type="button" onClick={() => setCustomPrimary('')}
                        className="text-xs text-slate-500 hover:text-white">Réinitialiser</button>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-8">
              {step > 0 && (
                <button type="button" onClick={() => setStep(s => s - 1)}
                  className="flex-none h-11 px-5 rounded-xl font-medium text-slate-300 hover:text-white
